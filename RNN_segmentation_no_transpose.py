...
with strategy.scope():
    ...
    callbacks = [
       ModelCheckpoint(f"{config['base_path2']}//{fname}//{os.environ['COMPUTERNAME']}_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
        ReduceLROnPlateau(monitor="val_loss", factor=0.1, patience=10, min_lr=0.000005, verbose= 1),
...
model = tf.keras.models.load_model(f"{config['base_path2']}//{fname}//{os.environ['COMPUTERNAME']}_Checkpoint{time_stamp}.h5",
                                   custom_objects={'TransformerEncoder':TransformerEncoder,"PatchEncoder":PatchEncoder})
...
for _ in range(10):
    ...
    # Predict
    pred0 = model.predict ( np.expand_dims(np.transpose(test_data),axis=0) ).squeeze()    
