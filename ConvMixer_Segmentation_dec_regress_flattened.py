...
model_name = 'ConvMixer_decimated_regress_flattened_2nd_try'

use_wandb = True
time_stamp = '24_April_23_0135' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
...
# Hyperparameters
config['Run_Note'] = 'ConvMixer_Segmentation_decimated_try2'
config['batch_size'] = 4
...
def regress_read_mat(filepath):
    def _read_mat(filepath):
        ...
        layer3D = tf.cast(mat_file['regress_32'], dtype = dtype) 
...
def get_conv_mixer_256_8(
     filters=64, depth=5, kernel_size=(17,15), patch_size=(config['img_y'],1), num_classes=config['num_classes']): #depth=8, kernel_size=5  patch_size=2,
    ...
    combined_out = layers.Conv2D( 1, (1,1), padding="same", dtype = tf.float64)(combined_out) # activation='sigmoid', name= "raster3D", 
    
    combined_out = layers.GlobalAveragePooling2D()(combined_out)
    combined_out = layers.Dropout(0.3) (combined_out)
    combined_out = layers.Dense(config['num_layers'] * config['img_x'], name= "raster3D", activation="sigmoid", dtype = tf.float64)(combined_out)
    
    

    return Model(inputs, outputs = [bin_output,combined_out ])
