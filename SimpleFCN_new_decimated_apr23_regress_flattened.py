...
model_name = 'SimpleFCNet_NewDecimated_Apr23_regress_2D_doubleGPU'

time_stamp = '11_May_23_2032' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
use_wandb = True
if use_wandb:    
    ## WandB config
    import wandb
...
with strategy.scope():
    ...
    regress_out = layers.Conv2D( 1, (1,1), padding="same", dtype = tf.float64, activation='relu' )(regress_out) # activation='sigmoid', name= "raster3D", 
    ...
    layer_diff_out = layers.Dense(config['num_layers'] * config['img_x'], name= "layer_diff", activation="sigmoid", dtype = tf.float64)(layer_diff_out)
