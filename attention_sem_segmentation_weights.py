# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 15:34:26 2022
...
def Create_ListDataLoader( img_paths,seg_type = 'segment' ):
    ...
    print(f'Target shape after categorical{y_train.shape}')
...
x_train,y_train = Create_ListDataLoader(train_input_img_paths)
x_val,y_val = Create_ListDataLoader(val_input_img_paths)
x_test,y_test = Create_ListDataLoader(test_img_paths)
...
class FocalLoss(tf.losses.Loss):
    ...
    #         })
    #     return config
custom_loss = FocalLoss(alpha = 0.5, gamma =2. )
...
with strategy.scope():
    ...
    # input_shape2 = (y_train.shape[1:])
    # weight_input2 = tf.keras.Input(shape=input_shape2)     
    
    #pos_inputs = tf.keras.Input(shape= input_shape)
    # pos_inputs = tf.range(input_shape[0], dtype='float64') 
    # pos_embedding = layers.Embedding(input_dim =  input_shape[0], output_dim = input_shape[-1])(pos_inputs)
    ...
    x = layers.Conv2D(num_classes*5, (1,1), activation="relu",padding="same", kernel_initializer='he_normal')(x)
    x = layers.Conv2D(num_classes, (1,1), activation="relu",padding="same", kernel_initializer='he_normal')(x)
    x = layers.Normalization()(x) 
    output = layers.Conv2D(num_classes, (1,1), activation="softmax",padding="same")(x)  
    
    model = tf.keras.Model(echo_inputs, output) 
    
    opt = tf.keras.optimizers.Adam(learning_rate=config['learning_rate'], clipnorm=0.1, clipvalue = 1.)
    ...
    model.compile(optimizer=opt,
                  loss=custom_loss, #tf.keras.losses.CategoricalCrossentropy(), #custom_loss, dice_coef_loss tf.keras.losses.CategoricalCrossentropy()
                  metrics=['accuracy'])

history = model.fit(train_ds, epochs = config['epochs'],validation_data = val_ds, callbacks = callbacks)

# Load best checkpoint
model = tf.keras.models.load_model(f"{config['base_path2']}//{fname}//{os.environ['COMPUTERNAME']}_Checkpoint{time_stamp}.h5",custom_objects={'TransformerEncoder':TransformerEncoder})

# Save model with proper name
_,acc = model.evaluate(np.transpose(x_test,axes=(0,2,1)),y_test_1hot)
