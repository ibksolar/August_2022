...
model_name = 'SimpleRes50_regress'

time_stamp = '17_May_23_2032' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
use_wandb = True
if use_wandb:    
    ## WandB config
    import wandb
...
regress_out = layers.MaxPool2D(pool_size=(13,1)) (regress_out1)
...
regress_out2 = layers.Conv2D( 16, (1,1), padding="same", activation='relu',)(regress_out)
...
model.compile(optimizer= opt, loss=  ["binary_crossentropy", "mean_squared_error", "mean_squared_error"], metrics=['accuracy']) # sparse_categorical_crossentropy,jaccard_distance,binary_crossentropy,tf.keras.losses.KLDivergence(),
