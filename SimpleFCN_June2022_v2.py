...
if use_wandb:
    ...
    wandb.init( project="my-test-project", entity="ibksolar", name='SimpleFCN_June2022'+time_stamp,config ={})
    config = wandb.config
else:
    config={}
...
model.compile(optimizer= 'Adam', loss= loss, metrics=['accuracy']) # sparse_categorical_crossentropy,jaccard_distance,binary_crossentropy,tf.keras.losses.KLDivergence(),
...
for idx in range(1,10):
  ...
  axarr[1].imshow(res0_final, cmap=custom_cm )
  axarr[1].set_title('Prediction')
  
  axarr[2].imshow(a_gt0.squeeze(),cmap=custom_cm) # gt
