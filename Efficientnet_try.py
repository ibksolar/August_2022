...
# define model
inputs = layers.Input(shape=(img_hgt, img_wdt, 3))
modelB7 = EfficientNetB7(weights='imagenet', drop_connect_rate=0.4, include_top=False)

modelB7.trainable = False
modelB7_ouput = modelB7(inputs)
# Convolution
x = layers.Conv2D(32,3,padding="same",activation='relu')(modelB7_ouput)
x = layers.Conv2D(64,3,padding="same",activation='relu')(x)

# Upsample
up1 = layers.UpSampling2D(size=(3,3))(x)
up1 = layers.Conv2D(32,3,padding="same",activation='relu')(up1)
up1 = layers.Conv2D(64,3,padding="same",activation='relu')(up1)

# Downsample
down1 = layers.MaxPooling2D(3,padding="same")(x)
down1 = layers.Conv2D(32,3,padding="same",activation='relu')(down1)
down1 = layers.Conv2D(64,3,padding="same",activation='relu')(down1)

# Merge resolutions and flatten

#(a) Downsample the upsampled image
up1_down = layers.MaxPool2D(pool_size=(3,3))(up1)
down1_up = layers.UpSampling2D(size=(3,3))(down1)

#(b) # Intermediate input - Add, convolve, flatten
intmd_inputs = layers.add([x,up1_down,down1_up])
intmd_inputs = layers.LayerNormalization(epsilon=1e-5)(intmd_inputs)

intmd_inputs = layers.Conv2D(32,3,padding="valid",activation='relu')(intmd_inputs)
intmd_inputs =layers.Conv2D(64,3,padding="valid",activation='relu')(intmd_inputs)

# Segment output layer
# output = layers.Conv2D(num_classes, (1, 1), activation='softmax' )(intmd_inputs)  #,    #layers.Dense(num_classes)(weighted_representation)
output = layers.Conv2D(1, (1, 1), activation='sigmoid' )(intmd_inputs) 
...
model_save_path = f'{base_dir}/EfficientNet_Bin_segmentation/SM_{Eff_seg_Acc*100: .2f}_{time_stamp}.h5'
