...
use_wandb = True
time_stamp = '15th_April_23_1809' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M') #'13_December_22_2204'
...
train_path = os.path.join(base_path,'larger_unsuper_data\*.mat')

# Create tf.data.Dataset
config['Run_Note'] = 'April 15th run of VAE'
config['batch_size'] = 4
...
def get_encoder(latent_dim=16):
    ...
    in1 = layers.Conv2D(3, (3, 3), activation='relu', padding='same', dtype =used_dtype )(encoder_inputs) #, kernel_initializer= kernel_init
    in1 = ResNet_50_model(in1)
    in1 = tf.cast(in1, dtype=used_dtype)
    
    x = layers.Conv2D(32, 3, activation="relu", strides=2, padding="same")(in1)
...
class VQVAETrainer(keras.models.Model):
    def __init__(self, train_variance=1, latent_dim=32, num_embeddings=128, **kwargs):
        super().__init__(**kwargs)
...
def read_mat(filepath):
    ...
    output = tf.py_function(_read_mat,[filepath],[tf.float32, tf.int32]) #,tf.double,tf.half, tf.float32,
    shape = output[1]
...
data_variance = 0.1 # np.var(train_ds)

logz= f"{config['base_path']}/{model_name}/{'time_stamp'}_logs/"
callbacks = [
   ModelCheckpoint(f"{config['base_path']}//{model_name}//{model_name}_Checkpoint_{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
...
vqvae_trainer = VQVAETrainer(latent_dim= latent_dim, num_embeddings = num_embeddings) #data_variance,
vqvae_trainer.compile(optimizer=keras.optimizers.Adam())

history = vqvae_trainer.fit(train_ds, epochs=100, batch_size=64, callbacks=callbacks)
