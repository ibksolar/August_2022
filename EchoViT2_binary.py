# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 00:49:46 2022
...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun  5 20:41:05 2022
EchoViT2 - FastTimeEmbed_binary
...
if use_wandb:
    ...
    wandb.init( project="my-test-project", entity="ibksolar", name='EchoViT2_binary'+time_stamp,config ={})
    config = wandb.config
    
    config['notes']='Previos EchoViT_Binary with activity regularizer did well but setting prediction threshold was tricky. Removing actv. regularizer: UPDATE: New method not training well yet'
else:
    config={}
...
def read_mat_train(filepath):
    ...
    data1 = output[1]   
    data1.set_shape([ 416,64,config['num_classes'] ])#,30,config['num_classes']    
    return data0,data1
...
def read_mat(filepath):
    ...
    data1 = output[1]   
    data1.set_shape([416,64,config['num_classes'] ]) #,30 ,2   
    return data0,data1
...
# Model hyper-param
config['embed_dim'] = embed_dim =  train_shape[0][1] #416
config['num_patches'] = num_patches= train_shape[0][2] #64  

transformer_layers = 20
config['num_heads'] = 20 #20
...
proj_head_units = [1024,512, 64] #2048,1024, 
mlp_head_units = [256,64]  # Size of the dense layers  #512,
...
def mlp(x, hidden_units, dropout_rate = config['dropout_rate']):
    for units in hidden_units:
        x = layers.Dense(units, activation=tf.nn.gelu, kernel_initializer='he_normal')(x)
...
for units in proj_head_units:
    dense_proj.add(layers.Dense(units, activation='relu', kernel_initializer='he_normal', use_bias=True) )
    
    
# inputs = dense_proj(inputs)    

# Encode patches
encoded_patches = FlippedPatchEncoder(num_patches=config['num_patches'] , embed_dim = config['embed_dim'])(x) # Flipped num_patches = 416
...
for _ in range(transformer_layers):
    ...
    # # MLP
    # mlp_out = mlp(x3,mlp_head_units+final_shape)    
    # # Skip connection 2.
    # encoded_patches = layers.Add()([x3, mlp_out])

representation = dense_proj(encoded_patches) # This is not used
representation = layers.LayerNormalization(epsilon=1e-6)(encoded_patches)

representation= tf.expand_dims(encoded_patches, axis=-1)  

representation = layers.Conv2D(config['num_classes']*5, (7,5), activation=tf.nn.gelu, padding="same")(representation)
representation = layers.Conv2D(config['num_classes']*5, (1,1), activation='relu', padding="same")(representation)
representation = layers.Conv2D(config['num_classes'], (1,1), activation="relu", padding="same")(representation)    
layers.Dropout(0.3)(representation)
#output = layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = tf.float32)(representation) #activation="softmax",
 
output = layers.Conv2D(1, (1,1), padding="same",activation="sigmoid", dtype = tf.float64)(representation) #, dtype = tf.float32, activity_regularizer='l2'
...
opt = tf.keras.optimizers.Adam(learning_rate=config['learning_rate']  ) #, clipnorm=0.5, , clipnorm=0.5
opt2 = tfa.optimizers.AdamW(weight_decay = 0.0001, learning_rate=config['learning_rate'])
opt3 = tf.keras.optimizers.SGD(learning_rate=config['learning_rate'],momentum = 0.9,clipnorm=0.5 )
...
callbacks = [
   ModelCheckpoint(f"{config['base_path']}//echo_vit2_binary//echo_vit2_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
...
loss = tf.keras.losses.CategoricalCrossentropy() #, custom_loss
model.compile(optimizer=opt,
          loss= total_loss, # total_loss, 'binary_crossentropy' #custom_loss, dice_coef_loss tf.keras.losses.CategoricalCrossentropy()
...
model = tf.keras.models.load_model(f"{config['base_path']}//echo_vit2_binary//echo_vit2_Checkpoint{time_stamp}.h5", custom_objects={"FlippedPatchEncoder":FlippedPatchEncoder})
...
model.save(f"{config['base_path']}//echo_vit2_binary//echo_vit2_binary{acc:.3f}_{time_stamp}.h5")
...
model_val_data_path = os.path.join(base_path,'new_test\*.mat')
...
for idx in range(10):
  ...
  axarr[1].imshow(res0_final, cmap='gray_r' )
  #axarr[2].imshow(res0_final, cmap='gray_r' )
  axarr[1].set_title('Prediction')

  axarr[2].imshow(a_gt0.squeeze(),cmap='gray_r') # gt
