...
output_base_path = r'Y:\ibikunle\Python_Project\Fall_2021\Predictions_Folder\DL_models_predictions_folder_final\all_L1_L2'
...
def read_mat_files(data_fp,prediction_fp):
    ...
    for file in glob.glob(prediction_fp):
        base_name = os.path.basename(file)
        
        output_file = os.path.join(data_fp,base_name)
        
        temp_test_file = loadmat(output_file) # GT
        temp_output = loadmat(file) # Now prediction
...
model_folders = ['AttUNet', 'DeepLab','ensemble','FCN','SimpleUNet','Res50_pretrained']
