...
model_name = 'echo_vit_regress'
use_wandb = True
time_stamp = '14_April_23_1303' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
...
config['Run_Note'] = 'Training Regression network 1D_Max_pool'
config['batch_size'] = 4
...
def read_mat(filepath):
    def _read_mat(filepath):
        ...
        layer3D = tf.cast( tf.cast(mat_file['regress_GT'], dtype=tf.bool), dtype = dtype)        
    ...
    data2 = output[2] 
    data2.set_shape([config['img_y']//52,config['img_x'], 1]) #,30, ,config['num_classes']    
...
with strategy.scope():
    ...
    # Expand dims to create combined output 
    # ==================================================================== 
   
    combined_out = layers.Conv2D(config['num_classes']*5, (17,15), activation='relu', padding="same", dtype = used_dtype) (combined)
    combined_out = layers.Conv2D(config['num_classes'], (17,15), activation='relu', padding="same", dtype = used_dtype) (combined_out)
    
    combined_out = layers.MaxPool2D(pool_size=(52,1)) (combined_out)
    combined_out = layers.Conv2D(128, 3, activation='relu', padding="same", dtype = used_dtype) (combined_out)
    combined_out = layers.Conv2D(64, 3, activation='relu', padding="same", dtype = used_dtype, activity_regularizer=keras.regularizers.L2(1e-5)) (combined_out)
    combined_out = layers.Dropout(0.3) (combined_out)
    combined_out = layers.Conv2D( 1, (1,1), padding="same", dtype = used_dtype, name= "raster3D", activation='sigmoid')(combined_out)
    ...
    model.compile(optimizer=opt2,#opt
             ...
             metrics=[ ['accuracy'] ,['mean_squared_error']  ] ) #metrics=['mean_absolute_error']                                           
