...
use_wandb = True
time_stamp = '15_January_23_1903' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M') 
...
if use_wandb:    
    ...
    wandb.init( project="my-test-project", entity="ibksolar", name='EchoViT_paper_BinMulti'+time_stamp,config ={})
    config = wandb.config
else:
    config ={}
...
config['Run_Note'] = 'Previous training was Nan: Changing opt to RMSprop and reducing learning rate'
config['batch_size'] = 4
...
def read_mat(filepath):
    def _read_mat(filepath):
        ...
        multi_layer = tf.cast(mat_file['semantic_seg2'], dtype = dtype) 
        multi_layer = tf.expand_dims(multi_layer, axis=-1)
        #layer = tf.keras.utils.to_categorical(layer, config['num_classes'] )
        
        shape0 = echo.shape        
        return echo,layer,multi_layer, np.asarray(shape0)     
    ...
    data2 = output[2]   
    data2.set_shape([config['img_y'],config['img_x'],config['img_channels']])
    
    return data0,{"row_embed_bin": data1, "row_embed_mult": data2,
                  "col_embed_bin": data1, "col_embed_mult": data2, 
                  "patch_embed_bin": data1, "patch_embed_mult": data2, 
                  "ResNet_embed_bin": data1, "ResNet_embed_mult": data2,  
                  "combined_mult": data2}
...
with strategy.scope():
    ...
    def transformer_unit(encoded_patches, transformer_layers = transformer_layers):    
        for _ in range(transformer_layers):
            ...
            # Feed forward
            mlp_out = mlp(x4,mlp_head_units,dropout_rate=0.1)
    ...
    pos_embed = layers.Embedding(input_dim = 2, output_dim = input_shape[1], input_length = input_shape[0] ) (tf.range(start=0,limit=input_shape[0],delta=1))
    ...
    combined = rw_transform + col_transform + patch_transform + ResNet_transform   

    
    used_activation = 'sigmoid' 
    multi_activation = 'softmax'
    ...
    # def output_convolution(output_name, bin_or_mult = 'bin'):
        
    #     used_activation = 'sigmoid' if bin_or_mult == 'bin' else 'softmax'
    #     out_class = config['num_classes'] if bin_or_mult == 'bin' else config['multi_classes']
    ...
    #     output_convolution.add( layers.Conv2D(out_class, (1,1), padding="same", dtype = used_dtype, activation= used_activation, name = output_name) )
    
    output_convolution = tf.keras.Sequential()
    output_convolution.add(layers.Conv2D(config['num_classes']*64, (17,15), activation='relu', padding="same", dtype = used_dtype) )    
    output_convolution.add( layers.Conv2D(config['num_classes']*64, 3, activation="relu", padding="same", dtype = used_dtype ) )
    output_convolution.add( layers.Dropout(config['dropout_rate']) )
    output_convolution.add( layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = used_dtype) )
    ...
    row_embed_bin_temp = output_convolution (rw_transform)
    row_embed_bin = layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = used_dtype, activation= used_activation, name = 'row_embed_bin') (row_embed_bin_temp)
    
    row_embed_mult = layers.Conv2D(1, (1,1), padding="same", dtype = used_dtype,activation='relu') (rw_transform)
    row_embed_mult = row_embed_mult + row_embed_bin_temp
    row_embed_mult = layers.LayerNormalization(epsilon=1e-6) (row_embed_mult)
    # row_embed_mult = layers.Conv2D(128, 1, padding="same", dtype = used_dtype, activation='relu') (row_embed_mult)
    row_embed_mult = layers.Conv2D(64, (1,1), padding="same", dtype = used_dtype, activation='relu') (row_embed_mult)
    row_embed_mult = layers.Conv2D(config['multi_classes'], (1,1), padding="same", dtype = used_dtype, activation= multi_activation, name = 'row_embed_mult') (row_embed_mult) #activation= multi_activation,
    
    
    # ==================================================================== 
    col_transform = tf.expand_dims(col_transform, axis=-1)
    col_embed_bin_temp = output_convolution(col_transform)
    col_embed_bin = layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = used_dtype, activation= used_activation, name = 'col_embed_bin')(col_embed_bin_temp)
    
    col_embed_mult = layers.Conv2D(1, (1,1), padding="same", dtype = used_dtype, activation='relu')(col_transform)
    col_embed_mult = row_embed_mult + col_embed_bin_temp
    col_embed_mult = layers.LayerNormalization(epsilon=1e-6) (col_embed_mult)
    # col_embed_mult = layers.Conv2D(128, 1, padding="same", dtype = used_dtype, activation='relu') (col_embed_mult)
    col_embed_mult = layers.Conv2D(64, (1,1), padding="same", dtype = used_dtype, activation='relu') (col_embed_mult)
    col_embed_mult = layers.Conv2D(config['multi_classes'], (1,1), padding="same", dtype = used_dtype, activation= multi_activation, name = 'col_embed_mult')(col_embed_mult)
    
    
    # ==================================================================== 
    patch_transform = tf.expand_dims(patch_transform, axis=-1)
    patch_embed_bin_temp = output_convolution (patch_transform) 
    patch_embed_bin = layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = used_dtype, activation= used_activation, name = 'patch_embed_bin')(patch_embed_bin_temp)
    
    patch_embed_mult = layers.Conv2D(1, (1,1), padding="same", dtype = used_dtype, activation='relu') (patch_transform) 
    patch_embed_mult = patch_embed_mult + patch_embed_bin_temp
    patch_embed_mult = layers.LayerNormalization(epsilon=1e-6) (patch_embed_mult)
    # patch_embed_mult = layers.Conv2D(128, 1, padding="same", dtype = used_dtype, activation='relu') (patch_embed_mult)
    patch_embed_mult = layers.Conv2D(64, (1,1), padding="same", dtype = used_dtype, activation='relu') (patch_embed_mult)
    patch_embed_mult = layers.Conv2D(config['multi_classes'], (1,1), padding="same", dtype = used_dtype, activation= multi_activation, name = 'patch_embed_mult')(patch_embed_mult)
    
    # ==================================================================== 
    ResNet_transform = tf.expand_dims(ResNet_transform, axis=-1)
    ResNet_embed_temp = output_convolution(ResNet_transform)
    ResNet_embed_bin = layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = used_dtype, activation= used_activation, name = 'ResNet_embed_bin')(ResNet_embed_temp)
    
    ResNet_embed_mult = layers.Conv2D(1, (1,1), padding="same", dtype = used_dtype, activation='relu') (ResNet_transform) 
    ResNet_embed_mult = ResNet_embed_mult + ResNet_embed_temp
    ResNet_embed_mult = layers.LayerNormalization(epsilon=1e-6) (ResNet_embed_mult)
    # ResNet_embed_mult = layers.Conv2D(128, 1, padding="same", dtype = used_dtype, activation='relu') (ResNet_embed_mult)
    ResNet_embed_mult = layers.Conv2D(64, (1,1), padding="same", dtype = used_dtype, activation='relu') (ResNet_embed_mult)
    ResNet_embed_mult = layers.Conv2D(config['multi_classes'], (1,1), padding="same", dtype = used_dtype,activation= multi_activation,  name = 'ResNet_embed_mult')(ResNet_embed_mult) #activation= multi_activation,
   
    
    combined = tf.expand_dims(combined, axis=-1)
    combined_out = output_convolution(combined)
    
    combined_mult = combined_out + combined
    combined_mult = layers.LayerNormalization(epsilon=1e-6) (combined_mult)
    # combined_mult = layers.Conv2D(128, 1, padding="same", dtype = used_dtype, activation='relu') (combined_mult)
    combined_mult = layers.Conv2D(64, (1,1), padding="same", dtype = used_dtype, activation='relu') (combined_mult)
    combined_mult = layers.Conv2D(config['multi_classes'], (1,1), padding="same", dtype = used_dtype,activation= multi_activation,  name = 'combined_mult')(combined_mult) #activation= multi_activation,
   
    
    
    outputs = [row_embed_bin, row_embed_mult, col_embed_bin, col_embed_mult, patch_embed_bin,patch_embed_mult, ResNet_embed_bin, ResNet_embed_mult, combined_mult ] #, combined_mult 
    ...
    logz= f"{config['base_path']}/EchoViT_paper/{config['start_time']}_logs/"
    callbacks = [
       ModelCheckpoint(f"{config['base_path']}//EchoViT_paper//EchoViT_BinMulti_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
    ...
    # loss = [sm.losses.BinaryFocalLoss(), SparseCategoricalFocalLoss(gamma=2)  ]*4 #+ [sm.losses.CategoricalFocalLoss()]
    
    loss = [sm.losses.BinaryFocalLoss(), tf.keras.losses.SparseCategoricalCrossentropy()  ]*4 + [ tf.keras.losses.SparseCategoricalCrossentropy() ]
    
    # loss = { "row_embed_bin":sm.losses.BinaryFocalLoss, "row_embed_mult": sm.losses.CategoricalFocalLoss, 
    #          "col_embed_bin":sm.losses.BinaryFocalLoss, "col_embed_mult": sm.losses.CategoricalFocalLoss, 
    #          "patch_embed_bin":sm.losses.BinaryFocalLoss, "patch_embed_mult": sm.losses.CategoricalFocalLoss, 
    #         } 
    
    model.compile(optimizer=opt,
              loss = loss, #SparseCategoricalFocalLoss(gamma=2), #custom_loss, dice_coef_loss tf.keras.losses.CategoricalCrossentropy()
              metrics=['accuracy'])                                             #,tf.keras.metrics.MeanIoU(num_classes, name="MeanIoU")
    
    config['history'] = model.fit(train_ds, epochs = config['epochs'],validation_data = val_ds, callbacks = callbacks)
...
loaded_model = tf.keras.models.load_model(f"{config['base_path']}//EchoViT_paper//EchoViT_BinMulti_Checkpoint{time_stamp}.h5")                                     
        

# Save model with proper name
_ = model.evaluate(test_ds)
model.save(f"{config['base_path']}//EchoViT_paper//EchoViT_BinMulti_{time_stamp}.h5")
...
output_names = [ 'row_embed_bin', 'row_embed_mult', 'col_embed_bin', 'col_embed_mult', 'patch_embed_bin', 'patch_embed_mult','ResNet_embed_bin', 'ResNet_embed_multi','Col_multi'  ]
