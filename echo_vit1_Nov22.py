...
def read_mat(filepath):
    def _read_mat(filepath):
        ...
        if config['num_classes'] == 1:
            ...
            layer = tf.cast(mat_file['semantic_seg2'], dtype = dtype) 
            layer = tf.expand_dims(layer, axis=-1)
            #layer = tf.keras.utils.to_categorical(layer, config['num_classes'] )
    ...
    output = tf.py_function(_read_mat,[filepath],[tf.half,tf.half, tf.int32]) #,tf.double
    shape = output[2]
    data0 = tf.reshape(output[0], shape)
...
class PatchEncoder2(layers.Layer):
    def __init__(self, num_patches = config['num_patches'], embed_dim = config['embed_dim'], **kwargs):
        super(PatchEncoder2, self).__init__(**kwargs)
        self.num_patches = num_patches
    ...
    def call(self, patch):
        ...
        #encoded = patch + tf.transpose(pos_embed,perm=[1,0])
        return pos_embed
...
with strategy.scope(): 
    ...
    encoded_patches = tf.cast( x + pos_embed , dtype=used_dtype)
    # encoded_patches = PatchEncoder2(num_patches, embed_dim)(x)
    ...
    for _ in range(transformer_layers):
        ...
        # Skip connection 2.
        encoded_patches =  mlp_in #layers.Add()([x3, mlp_out])
        # # # MLP (Newly Added: Might delete)
    ...
    representation = layers.Conv2D(config['num_classes']*5, (1,1), activation='relu', padding="same", dtype =used_dtype)(representation)
    # representation = layers.Conv2D(config['num_classes'], (1,1), activation="relu", padding="same", )(representation)   # activity_regularizer='l2', kernel_regularizer=tf.keras.regularizers.L1L2(l1=1e-5, l2=1e-4), 
    layers.Dropout(0.3)(representation)
    output = layers.Conv2D(config['num_classes'], (1,1),  dtype = used_dtype, activation = used_activation, padding="same")(representation) #activity_regularizer='l2',
    ...
    callbacks = [
       ModelCheckpoint(f"{config['base_path']}//echo_vit//echo_vit_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
    ...
    model.compile(optimizer=opt,#opt
              loss = loss, #SparseCategoricalFocalLoss(gamma=2), #SparseCategoricalFocalLoss(gamma=2), #tf.keras.losses.CategoricalCrossentropy(), #custom_loss, dice_coef_loss tf.keras.losses.CategoricalCrossentropy()
...
if type(loss) is not keras.losses.CategoricalCrossentropy:
    loaded_model = tf.keras.models.load_model(f"{config['base_path']}//echo_vit//echo_vit_Checkpoint{time_stamp}.h5",
                                   custom_objects={"PatchEncoder2":PatchEncoder2,"FocalLoss":FocalLoss})
else:
    loaded_model = tf.keras.models.load_model(f"{config['base_path']}//echo_vit//echo_vit_Checkpoint{time_stamp}.h5",
                                       custom_objects={"PatchEncoder2":PatchEncoder2})
        

# Save model with proper name
_,acc = model.evaluate(test_ds)
model.save(f"{config['base_path']}//echo_vit//echo_vit{acc:.2f}_{time_stamp}.h5")
