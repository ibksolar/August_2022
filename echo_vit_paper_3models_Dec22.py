...
use_wandb = True
time_stamp = '13_December_22_2204' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
...
if use_wandb:    
    ...
    wandb.init( project="my-test-project", entity="ibksolar", name='EchoViT_paper'+time_stamp,config ={})
    config = wandb.config
else:
    config ={}
...
with strategy.scope():
    ...
    output_convolution.add( layers.Conv2D(config['num_classes'], (1,1), padding="same", dtype = used_dtype, activation= used_activation) )
    ...
    outputs = [rw_embed_out, col_embed_out, patch_embed_out, ResNet_embed_out, combined_out  ]
    ...
    callbacks = [
       ModelCheckpoint(f"{config['base_path']}//EchoViT_paper//EchoViT_paper_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
...
if type(loss) is not keras.losses.CategoricalCrossentropy:
    loaded_model = tf.keras.models.load_model(f"{config['base_path']}//EchoViT_paper//EchoViT_paper_Checkpoint{time_stamp}.h5")
...
model.save(f"{config['base_path']}//EchoViT_paper//EchoViT_paper{acc:.2f}_{time_stamp}.h5")
