...
for file in glob.glob(os.path.join(base_path,'*.mat')):
    ...
    if 1: #base_name in ['20120330_04_0220_5km.mat']: # np.sum(raster) > 0 and  base_name in ['20120330_04_0207_5km.mat']: #'20120330_04_0220_5km.mat',, '20120330_04_0208_5km.mat', 20120330_04_0207_5km.mat,base_name in ['20120330_04_0207_5km.mat'] and
        aa = loadmat(file)     
        ...
        for ix in range(len(loc_idx)-1):
            ...
            if len( range(curr_loc,next_loc) ) <= Nx:
                ...
                while curr_loc + 1 < next_loc:
                    ...
                    tent_cols = bin_cols[ search_idx[:max_loc+1] ] 
                    
                    first_chk = np.mean( bin_rows[ used_rw_idx[np.argsort(tent_cols)[:10]]] )
                    ...
                    if max_loc < Nx or np.abs(max_val) <= 3: # (max_loc+next_max_loc) <= Nx: #
                    # if max_loc < Nx-1: #or max_loc < 5 #np.abs(max_val) < 10 and
                        # Sort and use columns                      
                        search_idx = remaining[ :min(round(3*Nx),stop_srch) ] if len(remaining) >=Nx-1 else remaining[:] # Expand search
                        ...
                        for iter in range(Nx): 
                            ...
                            if idx.ndim and idx.size : #or np.any(idx) or idx.item() == 0
                                idx = idx[0]
                                idx = idx.item() if len(idx) < 2 else [ idx_elem.item() for idx_elem in idx]
                                
                                decider = first_chk if not idx_list else bin_rows[search_idx[idx_list[-1]]]
                                
                                # If there are multiple candidates
                                if idx_list and len(idx) > 1:
                                    pot_bin_rws = [bin_rows[elem] for elem in idx] # bin rows of potential candidates
                                    idx = idx[np.argmin(abs(decider-pot_bin_rws))] # use the closest bin_rows                                                                     
                         
                                iter_list = list(range(iter)) if iter>0 else 0  # range(max(1,iter-30),iter)
                                ...
                                if np.abs( bin_rows[search_idx[idx]] - decider ) <= threshold  or ~np.ma.any( np.abs(bin_rows[search_idx[idx]] - bin_rw_b4) <=3 ) :
        ...
        if 1:
            save_fig_path = os.path.join(base_path,'plotted_images3',base_name)
