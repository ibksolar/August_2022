...
TODO:
    I think this is fixed (10/12/2022)
...
if use_wandb:    
    ...
    wandb.init( project="my-test-project", entity="ibksolar", name='AttentionUNet_binary_aug_large_focalLoss'+time_stamp,config ={})
    config = wandb.config
else:
    config ={}
...
config['num_classes'] = 1 # This needs to be UP here for dataset creation
...
img_size = (config['img_y'],config['img_x']) # :( Almost redundant

input_shape = img_size + (config['img_channels'] ,)

# input_shape = (None, None, config['img_channels'],) #, img_channels)
...
def double_conv_layer(x, filter_size, size, dropout, batch_norm=False):
    ...
    axis = 3
    conv = layers.Conv2D(size, (filter_size1, filter_size2), padding='same' , kernel_initializer=kernel_init,)(x)
    if batch_norm is True:
        conv = layers.BatchNormalization(axis=axis)(conv)
    conv = layers.Activation('relu')(conv)
    conv = layers.Conv2D(size, (filter_size1, filter_size2), padding='same', kernel_initializer=kernel_init)(conv) # Sreeni removed this
    if batch_norm is True:
        conv = layers.BatchNormalization(axis=axis)(conv)
    conv = layers.Activation('relu')(conv)
    if dropout > 0:
    ...
    shortcut = layers.Conv2D(size, kernel_size=(1, 1), padding='same', kernel_initializer=kernel_init)(x)
    if batch_norm is True:
...
def attention_block(x, gating, inter_shape, name):
    ...
    theta_x = layers.Conv2D(inter_shape, (2, 2), strides=(2, 2), padding='same', kernel_initializer = kernel_init)(x)  # 16
    ...
    psi = layers.Conv2D(1, (1, 1), padding='same', kernel_initializer = kernel_init)(act_xg)
...
def Attention_ResUNet_PA(input_shape,num_classes= config['num_classes'] ,dropout_rate=0.0, batch_norm=True):
    ...
    if config['num_classes'] >1:
        ...
        conv_final = tf.keras.layers.Conv2D(config['num_classes'] , (1, 1), activation='sigmoid', dtype=tf.float64)(up_conv_128) #relu, sigmoid, softmax
...
model = tf.keras.models.load_model(f"{config['base_path']}//AttUNet//AttUNet_Checkpoint{time_stamp}.h5", custom_objects={'binary_focal_loss':sm.losses.BinaryFocalLoss()})

_, accuracy = model.evaluate(test_ds)
model_save_path = f"{config['base_path']}//AttUNet//AttUNet_Acc{accuracy}_{time_stamp}.h5"
...
# ============================================================================
# Scripts to binarize and fix model output
# ============================================================================

import custom_binarize
import make_vec_layer
# ============================================================================
...
for idx in range(1,10):
  ...
  res0_final = np.where(res0>0.19,1,0) 
  res0_final1 = np.arange(1,416*4+1).reshape(416*4,1) * custom_binarize(res0,res0_final)
  
  b = np.ones((5,))/5; a = 1    

  # How correct is create_vec_layer??
  thresh = {'exp': 15}
  z = make_vec_layer(res0_final1,thresh); 
