...
if use_wandb:    
    ...
    wandb.init( project="my-test-project", entity="ibksolar", name='EchoViT1_large'+time_stamp,config ={})
    config = wandb.config
else:
    config ={}
...
config['Run_Note'] = 'Not so heavy augment training'
config['batch_size'] = 4
...
def read_mat_heavy_aug(filepath):
    def _read_mat(filepath):
        ...
        if tf.random.uniform(())> 0.3:
            ...
            # # Random Add
            echo = echo + tf.random.normal(shape=(416,config['img_x'],config['img_channels']),stddev=0.5,dtype=tf.float64)
            #echo = tf.clip_by_value(echo, 0, 1)  
    ...
    data0.set_shape([416,config['img_x'],config['img_channels']])
    
    data1 = output[1]   
    data1.set_shape([416,config['img_x'],30])#,30    
    return data0,data1

# Copied from SimpleUNet_Binary_large
# =============================================================================
# Function for training data
...
proj_head_units = [ 512, config['img_x']] #2048,1024,
...
class FocalLoss(tf.losses.Loss):
    ...
    def __init__(self, alpha = alpha, gamma = gamma,**kwargs):
        super(FocalLoss, self).__init__(
            reduction="none", name="Custom_FocalLoss"
        )
...
with strategy.scope():    
    ...
    representation = layers.Conv2D(config['num_classes']*5, (17,15), activation=tf.nn.gelu, padding="same")(representation)
    representation = layers.Conv2D(config['num_classes']*5, (7,7), activation='relu', padding="same")(representation)
    # representation = layers.Conv2D(config['num_classes'], 5, activation="relu", padding="same", )(representation)   # activity_regularizer='l2', kernel_regularizer=tf.keras.regularizers.L1L2(l1=1e-5, l2=1e-4), 
    layers.Dropout(0.3)(representation)
    output = layers.Conv2D(config['num_classes'], (1,1),  padding="same")(representation) # dtype = tf.float64, activity_regularizer='l2',activation="softmax",
    ...
    opt3 = tf.keras.optimizers.SGD(lr=0.01, momentum=0.9, clipnorm=1.0)
    ## Print Start time
    config['start_time'] = datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
    print(f' Start time {config["start_time"]}')
    
    
    logz= f"{config['base_path']}/echo_vit1_large/{config['start_time']}_logs/"
    callbacks = [
       ModelCheckpoint(f"{config['base_path']}//echo_vit1_large//echo_vit_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
...
if type(loss) is not keras.losses.CategoricalCrossentropy:
    loaded_model = tf.keras.models.load_model(f"{config['base_path']}//echo_vit1_large//echo_vit_Checkpoint{time_stamp}.h5",
...
model.save(f"{config['base_path']}//echo_vit1_large//echo_vit{acc:.2f}_{time_stamp}.h5")
