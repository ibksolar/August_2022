...
model_name = 'SimpleRes50_regress_attention_w_CosineLR'

time_stamp = '25_May_23_2023' #datetime.strftime( datetime.now(),'%d_%B_%y_%H%M')
...
base_path = r'M:\ibikunle\Python_Project\Fall_2021\all_block_data\Attention_Train_data\SR_Dataset_v1\Dec'
...
# Transformer and model hyper-params
# It's important to get this right
embed_dim =  config['trans_embed_dim'] = 32 # This is the number of layers we are expecting to predict
config['trans_input_shape'] = (embed_dim , config['img_x'])

num_heads = 12
dense_dim = 512
mlp_head_units = [512,256]

transformer_units = [embed_dim * 4, embed_dim,]
transformer_layers = 10



# =============================================================================
def regress_read_mat_2_output(filepath):
    def _read_mat(filepath):
        ...
        layer_diff = tf.cast(mat_file['layer_diff'], dtype = dtype) 
        # layer_diff = tf.reshape(layer_diff, shape = (1,1,-1) )
        layer_diff = tf.expand_dims(layer_diff, axis=-1)
        
        shape0 = echo.shape        
        return echo,layer,layer_diff, np.asarray(shape0)     
    
    output = tf.py_function(_read_mat,[filepath],[tf.double,tf.double,tf.double, tf.int64]) #,tf.double, tf.half
    shape = output[3]
    ...
    return data0,{'raster':data1, 'layer_diff':data2}
...
train_ds = train_ds.map(regress_read_mat_2_output,num_parallel_calls=8)
...
val_ds = val_ds.map(regress_read_mat_2_output,num_parallel_calls=8)
...
test_ds = test_ds.map(regress_read_mat_2_output,num_parallel_calls=8)
...
# Customized Learning rate for Callback
...
ResNet_50_model = sm.Unet(backbone_name='resnet101', encoder_weights='imagenet', encoder_freeze=True, input_shape = (None,None,3) ) #   
...
x = tf.keras.layers.Conv2D(16, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(in2)
...
# Regression output
regress_init = layers.Conv2D(256, 3, activation='relu', padding="same") (x)
# regress_init = layers.Conv2D(128, 5, activation='relu', padding="same") (regress_init)

regress_init = layers.MaxPool2D(pool_size=(13,1)) (regress_init)
...
regress_out1 = tf.keras.layers.BatchNormalization()(regress_out1)


regress_out = layers.Conv2D( 1, (1,1), padding="same" )(regress_out1)

############### TRANSFORMER STUFF ##########################################
# Dimension has changed here - it's now number of output_rows X num_cols

## Row Embed   
rw_embed = layers.Conv2D(1, (1,config['img_x']), activation='relu',  padding='same' , dtype =used_dtype)(rows_out) 

## Column Embed
col_embed = layers.Conv2D(1, (embed_dim,1), activation='relu',  padding='same' , dtype =used_dtype)(cols_out) #kernel_initializer= kernel_init,
...
pos_embed = layers.Embedding(input_dim = 120, output_dim = config['trans_input_shape'][1], input_length = config['trans_input_shape'][0] ) (tf.range(start=0,limit=config['trans_input_shape'][0],delta=1))
pos_embed = tf.cast(pos_embed, dtype =used_dtype)

# (a.) Row embed + pos_encode
rw_embed = tf.reduce_mean(rw_embed,axis = -1)
...
regress_out = rw_transform + col_transform #regress_out +
## Do another normalization here?
regress_out = tf.keras.layers.BatchNormalization()(regress_out)

# regress_out = layers.Conv2D( 16, (1,1), padding="same", activation='relu',)(regress_out)
# regress_out = layers.Dropout(0.3) (regress_out)
# regress_out = layers.Conv2D( 1, (1,1), padding="same", dtype = tf.float64, activation='linear', name= "raster3D", )(regress_out) # activation='sigmoid', name= "raster3D", 

regress_out2 = layers.Conv2D( 128, (1,1), padding="same", activation='relu',)(regress_out)
regress_out2 = layers.Conv2D( 16, (1,1), padding="same", activation='relu',)(regress_out2)
regress_out2 = layers.Dropout(0.3) (regress_out2)
layer_diff_out = layers.Conv2D( 1, (1,1), padding="same", dtype = tf.float64, activation='linear', name= "layer_diff", )(regress_out2) 

# regress_out = layers.Dense(config['num_layers'] * config['img_x'], name= "raster3D", activation="sigmoid", dtype = tf.float64)(regress_out)



model = tf.keras.Model(inputs,[bin_output, layer_diff_out] )
opt = tfa.optimizers.AdamW(learning_rate=config['learning_rate'], weight_decay = config['weight_decay'])
model.compile(optimizer= 'Adam', loss=  ["binary_crossentropy", "mean_squared_error"], metrics=['accuracy', "mean_squared_error"]) # sparse_categorical_crossentropy,jaccard_distance,binary_crossentropy,tf.keras.losses.KLDivergence(),
...
callbacks = [
   ModelCheckpoint(f"{config['base_path']}//{model_name}//{model_name}_Checkpoint{time_stamp}.h5", save_best_only=True, monitor="val_loss"),
    ReduceLROnPlateau(monitor="val_loss", factor=0.25, patience=25, min_lr=0.000005, verbose= 1),
    EarlyStopping(monitor="val_loss", patience=30, verbose=1), 
    TensorBoard(log_dir = logz,histogram_freq = 1,profile_batch = '1,70', embeddings_freq=50),
    WandbCallback(),
    WarmupCosineDecay(total_steps= total_steps, warmup_steps=warmup_steps, hold=int(warmup_steps/2),start_lr=0.0, target_lr=1e-3)
]
